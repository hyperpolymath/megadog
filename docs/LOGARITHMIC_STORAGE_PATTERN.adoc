...
[arabic]
. *Diffs are smaller*: Δln(x) is small even when Δx is huge
. *Merkle proofs smaller*: Smaller values = smaller proofs
. *Compression works better*: Log values have more redundancy
...
*At 1M updates/day on Polygon:* - Traditional: ~$500/day - Batched log:
~$8/day - *Annual savings: ~$180,000*
...
Logarithmic storage is: - *Proven*: Used in audio, finance, databases
for decades - *Underutilized*: Blockchain devs haven’t adopted it widely
- *Significant*: 50%+ savings at scale - *Safe*: Exact for most
operations, bounded error for addition

If your values grow exponentially and you don’t need exact precision,
use it.
