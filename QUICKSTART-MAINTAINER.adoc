...
// Replace megadog, megadog, {{DEPS}} with actuals
= megadog — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining megadog for
...
| `$PREFIX/share/megadog/`
| Data files, assets

| `$PREFIX/share/doc/megadog/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/megadog/config.toml`

Fallback: `$HOME/.config/megadog/config.toml`
...
Or via OPSM: `opsm update megadog`
...
[source,bash]
----
just install --prefix=/opt/megadog-instance1 --config=/etc/megadog/instance1.toml
just install --prefix=/opt/megadog-instance2 --config=/etc/megadog/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/megadog/issues
