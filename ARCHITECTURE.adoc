...
*Responsibilities:* - Game UI and touch handling - GPU-accelerated
Mandelbrot rendering - Local state caching - WebSocket communication
with server

*Key Classes:* - `+MandelbrotRenderer+`: OpenGL ES raymarching for 3D
fractals - `+DogManager+`: Local dog state management - `+GameClient+`:
WebSocket connection handler - `+WallpaperExporter+`: High-res fractal
export
...
*Why Pony:* - Actor isolation prevents data races - Reference
capabilities enforce memory safety - No GC pauses (critical for game
servers) - Type-safe concurrency
...
*Contract Functions:* - `+mint_starter_dog()+`: Create initial dog for
new players - `+merge_dogs()+`: Combine two same-level dogs -
